...
import AppSidebar from '../../../compornent/layout/Student/StudentSidebar.jsx';
...
const ProjectDetails = () => {
    ...
    const handleConfirmDelete = () => {
        if (recordToDelete) {
            ProjectBacklogAPI.deleteProjectBacklogs(recordToDelete.functionName, recordToDelete.feature, projectId, recordToDelete.screenName)
                .then(() => {
                    openNotificationWithIcon('success', 'Delete failed');
                    refreshProjects();
                })
                .catch(error => {
                    const errorMessage = error.response?.data?.message || error.response?.data || 'Failed to Delete. Please try again later.';
    ...
    const save = () => {
        ...
        // if (!editingRecord.featureId) {
        //     openNotificationWithIcon('warning', 'Please select a feature before updating.');
        //     return;
        // }
        ...
        ProjectBacklogAPI.updateProjectBacklogs(
            combinedDTO
        )
            .then(response => {
            ...
            .catch(error => {
                console.log(error)
                const errorMessage = error.response?.data?.message || 'Failed to update. Please try again later.';
            ...
            <Layout>
                ...
                <Content style={{ padding: '20px 140px' }}>
                    ...
                    <Descriptions bordered layout="horizontal">
                        ...
                        <Item label={<strong>Created At</strong>}>{formatDate(project.createdAt)}</Item>
                        <Item label={<strong>Updated At</strong>}>{formatDate(project.updatedAt)}</Item>
                        <Item label={<strong>Created By</strong>}>{project.createdBy.fullName}</Item>
                        <Item label={<strong>Updated By</strong>}>{project.updatedBy.fullName}</Item>
